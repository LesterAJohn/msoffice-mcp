...
function getRequestIp(req, trustedProxy) {
  if (trustedProxy) {
    ...
    const first = Array.isArray(forwardedFor) ? forwardedFor[0] : String(forwardedFor ?? "").split(",")[0];
...
function tokenMatches(tokenDigests, candidate) {
  ...
  return tokenDigests.some((knownDigest) => knownDigest.length === digest.length && timingSafeEqual(knownDigest, digest));
}
...
function writeJsonRpcError(res, statusCode, message, code = -32000) {
  writeJson(res, statusCode, { jsonrpc: "2.0", error: { code, message }, id: null });
}

function createAccessLogger({ logger = console }) {
  return ({ req, path, statusCode, startTime, requestId, ip }) => {
    const durationMs = Date.now() - startTime;
    logger.info(JSON.stringify({ event: "mcp_http_access", time: new Date().toISOString(), requestId, method: req.method, path, statusCode, durationMs, ip, userAgent: req.headers["user-agent"] ?? null }));
  };
}

export function createHttpMcpServer({ host, port, mcpPath, healthPath, authMode = "token", authTokens, tokenVerifier, oauth2Verifier, trustedProxy, allowedOrigins, allowedIps, maxBodyBytes, rateLimitWindowMs, rateLimitMaxRequests, createMcpServer, logger = console }) {
  ...
  const rateLimiter = createRateLimiter({ windowMs: Math.max(rateLimitWindowMs, 1), maxRequests: Math.max(rateLimitMaxRequests, 1) });
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      if (req.method === "GET" && path === normalizedHealthPath) {
        writeJson(res, 200, { ok: true, status: 200, transport: "http", path: normalizedMcpPath });
        finalize(200);
        return;
      }
      ...
      const tokenAuthorized = authMode === "token" || authMode === "both" ? (tokenVerifier ? (await tokenVerifier.verify(bearerToken)).ok : tokenMatches(tokenDigests, bearerToken)) : false;
      const oauth2Authorized = (authMode === "oauth2" || authMode === "both") ? (await oauth2Verifier.verify(bearerToken)).ok : false;
